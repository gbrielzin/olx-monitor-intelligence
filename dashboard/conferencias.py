...
def registrar_conferencia(
    ...
    with get_connection() as conn:
        ...
        conn.execute(
            "INSERT INTO conferencias (listing_id, titulo, preco, mediana_grupo, margem_pct, veredito, motivo, "
            "conferido_em, descricao) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (listing_id, titulo, preco, mediana_grupo, margem_pct, veredito, motivo,
             datetime.now(timezone.utc).isoformat(), (descricao or "").strip() or None),
        )
